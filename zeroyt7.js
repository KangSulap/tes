...
//━━━━━━━━━━━━━━━[ CONNECTION 1 ]━━━━━━━━━━━━━━━━━//
		...
		mess = {
			wait: 'Sabar Lagi Proses Tod \n Ga Sabar Mending Mati Ajalah...!',
			success: 'Done ye ngentod',
			error: {
...
break
	...
	Note : Fitur Masih Dikit Karena Gw Bukan Hacker Atau Pemrograman
	Jangan Lupa Bersyukur Kntl
`
	but = [
{ buttonId: `${prefix}listdiamond`, buttonText: { displayText: 'KLO KALIAN PENCET GA ADA APA APA' }, type: 1 },
...
costum(buffer, sticker, Verived, `Ngapain dibaca? `)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
buffer = fs.readFileSync(ran)
costum(buffer, image, Verived, `Ngapain Dibaca? `)
fs.unlinkSync(ran)
})
break
case 'tomp3':
...
fs.unlinkSync(anum)
buffer = fs.readFileSync(ran)
zeroyt7.sendMessage(from, buffer, video, { quoted: ftrol, caption: 'puas lu?'})
fs.unlinkSync(ran)
})
break
...
await zeroyt7.sendMessage(from, get_audio, video, { mimetype: 'video/mp4', filename: `${get_result.title}.mp4`, quoted: ftrol, caption: 'puas lu?'})
break
case 'tiktok':
if (args.length == 0) return reply(`Link Nya Mana Tod\nContoh: ${prefix + command} https://www.youtube.com/watch?v=qZIQAk-BUEc`)
ini_url = args[0]
...
await zeroyt7.sendMessage(from, ini_buffer, video, { quoted: ftrol, caption: 'puas lu?'})
break
case 'tiktokmusic':
...
await zeroyt7.sendMessage(from, ini_buffer, image, { quoted: ftrol, caption: 'puas lu?'})
break
...
reply(mess.wait)
zeroyt7.sendMessage(from, gambar, image, { quoted: ftrol, caption: 'puas lu?'})
})
break
case 'chiisaihentai':
case 'trap':
...
zeroyt7.sendMessage(from, buffer, image, {quoted: ftrol, caption: 'puas lu?'})
break
case 'asupanhijaber':
...
zeroyt7.sendMessage(from, goo, image, {quoted: ftrol, caption: 'puas lu'})
break
case 'cecanmalaysia':
...
DpinBot.sendMessage(`6285694982557@s.whatsapp.net`, options, text, { quoted: ftrol })
...
sendButton(from, "GOPAY : 085694982557 (Devin rangga)", faketeks, but, mek)
break
case 'dana':
but = [
{ buttonId: `${prefix}done`, buttonText: { displayText: 'DONE' }, type: 1 }
]
sendButton(from, "DANA : 085694982557 (Devin rangga)", faketeks, but, mek)
break
case 'ovo':
but = [
{ buttonId: `${prefix}done`, buttonText: { displayText: 'DONE' }, type: 1 }
...
break
case 'donasi':
txtt = `Mau Donasi Pake Apa Cuma Liat Doang ?
